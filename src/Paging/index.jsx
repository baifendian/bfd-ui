...
export default React.createClass({
  ...
  render() {
    ...
    for( let i = 1; i <= pageNum; i++ ) {
      if( i <= showPage ) {
        liArr.push( <li key={i} className={this.state.currentIndex === i ? '_active':''} onClick={this.handleClick.bind(this, i)}><a>{i}</a></li> )
        if( i == showPage ) {
      ...
      if( pageNum == i && pageNum>showPage) {
        liArr[ i ] = <li key={i+1} className={this.state.currentIndex === i ? '_active':''} onClick={this.handleClick.bind(this, i)}><a>{i}</a></li>
      }
      if( this.state.currentIndex + 1 > showPage ) {
        ...
        for( let o = everpage; o <= index + 1; o++ ) {
          if( o <= pageNum ) {
            k++
            currentArr[ k ] = <li key={k} className={this.state.currentIndex === o ? '_active':''} onClick={this.handleClick.bind(this, o)}><a>{o}</a></li>
          }else {
            lastPage = true
            break
          }
        }
        liArr[ 0 ] = <li key="01" className={this.state.currentIndex === 1 ? '_active':''} onClick={this.handleClick.bind(this, 1)}><a>1</a></li>
        ...
        if( !lastPage ) {
          if( index < (pageNum - 1) ) {
            ...
            liArr[ liArr.length ] = <li key={liArr.length} className={this.state.currentIndex === pageNum ? '_active':''} onClick={this.handleClick.bind(this, pageNum)}><a>{pageNum}</a></li>
          }
        }
        break
      }
      ...
      <div className="bfd-paging row">
        ...
        <div className="pull-right layout-right">
          <ul className="pagination">
            <li ><a onClick={this.handleLaquoClick}  className= {'prev '+(this.state.currentIndex===1?'frist':'')}   >上一页</a></li>
              {liArr}
            <li><a onClick={this.handleRaquoClick} className={'next '+(this.state.currentIndex===pageNum?'end':'') }>下一页</a></li>
          </ul>
